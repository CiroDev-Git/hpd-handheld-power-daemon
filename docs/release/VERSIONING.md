...
| **Polkit action IDs `dev.cirodev.hpd.{set-tdp, set-charge, set-profile, set-fan-curve}`** and the `wheel` grant in `49-hpd.rules` | Renaming an action ID; changing its default policy (`auth_admin` → `no` etc.); removing the `wheel` passwordless grant. |
